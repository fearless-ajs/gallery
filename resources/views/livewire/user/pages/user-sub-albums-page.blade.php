<div class="content full-height hor-content hor-content_padd">
    ...
    <div class="horizontal-grid-wrap  fl-wrap full-height ">
        ...
        <div   id="portfolio_horizontal_container" class="three-ver-columns lightgallery">
            ...
            @if($sub_albums)
                @foreach($sub_albums as $album)
                    <div class="portfolio_item {{$album->content}}">
                        <div class="grid-item-holder">
                            @if($album->content == 'Pictures')
                                <a href="{{route('user.sub-pictures', $album->id)}}"><img src="{{$album->ImagePath}}" alt=""></a>
                                <div class="thumb-info">
                                    <h3><a href="{{route('user.sub-pictures', $album->id)}}">{{$album->title}}</a></h3>
                                </div>
                            @endif
                            @if($album->content == 'Albums')
                                <a href="{{route('user.sub-sub-albums', $album->id)}}"><img src="{{$album->ImagePath}}" alt=""></a>
                                <div class="thumb-info">
                                    <h3><a href="{{route('user.sub-sub-albums', $album->id)}}">{{$album->title}}</a></h3>
                                </div>
                            @endif
                        </div>
                    </div>
